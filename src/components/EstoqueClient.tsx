...
export default function EstoqueClient({ vehicles }: Props) {
  ...
  const marcas = useMemo(() => [...new Set(vehicles.map((v) => v.marca))].sort(), [vehicles]);
  const tipos = useMemo(() => [...new Set(vehicles.map((v) => v.tipo))].sort(), [vehicles]);
  const anos = useMemo(() => [...new Set(vehicles.map((v) => v.ano))].sort((a, b) => b - a), [vehicles]);
  ...
  const filtered = useMemo(() => {
    ...
    list = list.filter((v) => v.preco <= precoMax || v.preco === 0);
    ...
    return list;
  }, [search, marcaFiltro, tipoFiltro, anoFiltro, precoMax, order, vehicles]);
    ...
    <div className="min-h-screen bg-zinc-50">
      ...
      <div className="bg-zinc-950 py-12 px-4">
        <div className="max-w-7xl mx-auto">
          <span className="text-orange-400 text-xs font-bold uppercase tracking-widest">Estoque</span>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 rounded-xl border border-zinc-200 bg-white focus:outline-none focus:border-orange-400 text-sm"
            />
          </div>
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-3 rounded-xl border text-sm font-semibold transition-all ${
              showFilters || hasFilters
                ...
                : "bg-white border-zinc-200 text-zinc-700 hover:border-orange-400"
            }`}
          >
            <SlidersHorizontal size={16} /> Filtros
            {hasFilters && (
              <span className="bg-zinc-950 text-orange-400 text-xs w-5 h-5 rounded-full flex items-center justify-center font-bold">
                !
              </span>
            )}
          </button>
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none bg-white border border-zinc-200 rounded-xl px-4 py-3 pr-9 text-sm font-medium text-zinc-700 focus:outline-none focus:border-orange-400 cursor-pointer"
            >
              {ORDER_OPTIONS.map((o) => (
        ...
        {filtered.length > 0 ? (
          ...
          <div className="py-20 text-center">
            ...
            <p className="text-zinc-400 mb-6">Tente ajustar os filtros ou a busca</p>
            <button onClick={clearFilters} className="bg-orange-500 text-white font-bold px-6 py-2.5 rounded-full">
              Limpar filtros
            </button>
          </div>
        )}
